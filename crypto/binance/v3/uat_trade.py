...
class StrategyHandler(Handler):
    ...
    def handleTimer(self, data:TimeridType):
        ...
        if self.count % 10 == 5:  # and self.count <= 2
            ...
            side = "buy"  
            self.client.insertMarketUOrder(gateway, self.symbol, float(qtyStr), side,self.flagDict['positionside'] )
...
# nohup python3 -u main_trade.py -n testtrade1 -s 8889 -c 39090 -X BTCUSDT -p 10m -w 40 -d 2 -T 0.5 -t 1000 >> log.txt 2>&1 &
