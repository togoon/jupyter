...
class StrategyHandler(Handler):
    ...
    def handleKline(self, data:KlineType):
        ...
        if len(ukdf) != 0 and self.closeSec >= int(ukdf.closeSec.iloc[-1]) + intervalSec and self.openSec < int(ukdf.closeSec.iloc[-1]) + 60:            
            ...
            if len( ukdf.loc[ukdf.tradeDate == self.tradeDate] ) == wid:  #
                ...
                panel = pymed(ukdf[ukdf.tradeDate == self.tradeDate],wid)
                ...
                df_panel.iloc[-1, df_panel.columns.get_loc('thd')] = df_panel.iloc[-1, df_panel.columns.get_loc('emd')] - df_panel['emd'].rolling(window=histDays).mean()[-1:].values[0]
                ...
                self.pct_pre = df_panel.iloc[-1, df_panel.columns.get_loc('pct_pre')]

                self.queryContractAssets()

                if self.thd > 0 and self.pct_pre > 0:  #open < close
    ...
    def insertFactor(self):
        ...
        self.client.insertFactor(curDateTime, f"name:{self.name}, symbol:{self.symbol}, tradeDate:{self.tradeDate}, closeTime:{self.closeTime}, close:{self.close}, total:{self.total}, pct_pre:{self.pct_pre}, thd:{self.thd}, side:{self.flagDict['side']} " )

        logger.info(f"curDateTime:{curDateTime}, name:{self.name}, symbol:{self.symbol}, tradeDate:{self.tradeDate}, closeTime:{self.closeTime}, close:{self.close}, total:{self.total}, pct_pre:{self.pct_pre}, thd:{self.thd}, side:{self.flagDict['side']} " )
...
def run(argv):
    ...
    # df_panel.reset_index(inplace=True) #err level_0
