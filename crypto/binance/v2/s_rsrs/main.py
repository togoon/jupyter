...
class StrategyHandler(Handler):
    ...
    def handleKline(self, data:KlineType):
        ...
        df_s['beta'] = df_s['index'].rolling(wid).apply(lambda x: func(x, df_s))
        df_s['zscore'] = (df_s['beta']-df_s['beta'].rolling(wid2).mean())/df_s['beta'].rolling(wid2).std()
        df_s['signal'] = np.where((df_s['zscore'] > thd), 1, np.where((df_s['zscore'] < -thd), -1, np.nan))  
        
        logger.info(f"df_s.iloc[-5:,:] :\n{df_s.iloc[-5:,:]}" )

        # self.getModel()
        self.queryPositions()
        
        curSign = df_s['signal'].iloc[-1].astype(int)
...
# python3 -u main.py -n s_rsrs -s 8808 -c 29097 -X BTCUSDT -p 4h -w 25 -w 90 -d 120 -T 0.6 -t 1000000
