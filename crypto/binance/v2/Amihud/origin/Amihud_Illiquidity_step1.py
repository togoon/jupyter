...
def loading_data(cate_id,wid_,thd,if_print,wid_back,thd_std):
    cost_ = -0.0016
...
if __name__ == '__main__':
    ...
    wid_back = np.arange(10)*5+10
    thd_std = np.arange(6)*0.5 + 0.5
    ...
    for index_para,row_para in tqdm(df_para.iterrows(),total=df_para.shape[0]):
        ...
        df_para['re'].iloc[index_para],df_para['std'].iloc[index_para] = loading_data('btc',wid_,thd,if_print,int(row_para[2]),row_para[3])
        # break
