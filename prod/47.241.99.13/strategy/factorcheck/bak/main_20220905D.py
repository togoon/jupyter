...
class StrategyHandler(Handler):
    ...
    def handleKline(self, data:KlineType):
        ...
        print('\n--handleKline--: ', f'{self.name=}, {self.symbol=}, {interval=}, {intervalSec=}, {wid=}, {wid2=}, {thd=}, {self.factorCnt=}, {self.factor=}, {self.count=}', '\n')
        ...
        if len(ukdf) != 0 and self.closeSec >= int(ukdf.closeSec.iloc[-1]) + intervalSec and self.openSec < int(ukdf.closeSec.iloc[-1]) + 60:            
            ...
            if ukdf['idx'].iloc[-1] == remainder: #5
                ...
                elif (self.factorCnt == wid2 -1 and self.flagDict['isOpen'])  or int(self.closeTime) >= 235559 or (self.flagDict['isOpen'] and (len(df_panel.index) - df_panel[df_panel['factor'] > float(thd) ].index.tolist()[-1] == wid2 -1) ):
