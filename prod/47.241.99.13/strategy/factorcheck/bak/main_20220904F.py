...
class StrategyHandler(Handler):
    ...
    def handleKline(self, data:KlineType):
        ...
        if len(ukdf) != 0 and self.closeSec >= int(ukdf.closeSec.iloc[-1]) + intervalSec and self.openSec < int(ukdf.closeSec.iloc[-1]) + 60:            
            ...
            elif (self.factorCnt == wid2 -1 and self.flagDict['isOpen'])  or int(self.closeTime) >= 235559 or (self.flagDict['isOpen'] and (len(df_panel.index) - df_panel['index'][df_panel['factor'] > thd ].index.tolist()[-1]  == wid2 -1) ):
