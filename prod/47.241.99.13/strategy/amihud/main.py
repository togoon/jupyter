...
wid = 144  #  42  108  83 40 25
wid2 = 25  # 90
thd = 0.8  # 阈值0  0.6  0.55 0.8
thd2 = 2.5  # 
...
class StrategyHandler(Handler):
    ...
    def handleKline(self, data:KlineType):
        ...
        ds = ukdf.tail(wid)
        amt_sum = np.sum(ds['amount']) # idx rolling(int(wid_) wid 144
        ds1 = ds[ds['indicator']>np.percentile(ds['indicator'],thd*100)]   # thd 0.8
        amt_pct = np.sum(ds1['amount'])

        value = amt_pct / amt_sum / (1-thd) # thd 0.8
        ukdf.iloc[-1, ukdf.columns.get_loc('value')] = value

        value_mean = np.mean(ukdf['value'].tail(wid * wid2) ) # wid 144 , wid2 25
        value_std = np.std(ukdf['value'].tail(wid * wid2) ) # wid 144 , wid2 25
        ukdf.iloc[-1, ukdf.columns.get_loc('value_mean')] = value_mean
        ukdf.iloc[-1, ukdf.columns.get_loc('value_std')] = value_std

        # signal = np.where((value > value_mean +thd2*value_std), 1, np.where((value < value_mean -thd2*value_std), -1, 0) ) # thd2 2.5
        signal = 1 if value > value_mean +thd2*value_std else ( -1 if value < value_mean -thd2*value_std else 0)  # thd2 2.5
...
def run(argv):
    ...
    ukdf['value'] = ukdf['index'].rolling(int(wid)).apply(lambda x: func(x, ukdf, thd)) # wid_ 144, thd 0.8

    ukdf['value_mean'] = ukdf['value'].rolling(int(wid*int(wid2)) ).mean()  # wid_back 25 , 3600
    ukdf['value_std'] = ukdf['value'].rolling(int(wid*int(wid2)) ).std()  # wid_back 25

    ukdf['signal'] = np.where((ukdf['value'] > ukdf['value_mean']+thd2*ukdf['value_std']), 1, np.where((ukdf['value'] < ukdf['value_mean']-thd2*ukdf['value_std']), -1, 0 ) )   # thd_std 2.5
