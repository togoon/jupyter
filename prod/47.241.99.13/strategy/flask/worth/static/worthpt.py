...
def getWorth1(nameList):
    ...
    for name in nameList:
        ...
        worthdf.groupby('date').last().plot( y='totalworth', title=f'{name}-worth-day', label=None, ax=ax1)  #, figsize=(5,3)
...
def getWorth2():
    ...
    for index, rows in infodf.iterrows():
        ...
        if 'test' not in name:
            csvdf2.loc[len(csvdf2.index)] = [name, sid, starttime, curtime, runtime, amtSingle, quota, worth, profitRate, tradeCount, profitRateYear, note]
...
def setworthhtml():
    ...
    with open('templates/worth.html', 'w') as f:
        f.write(html_string.format(tableEle1=csvdf1.to_html(classes='worth'), plotEle1=''.join(plotEleList1), tableEle2=csvdf2.to_html(classes='worth'), plotEle2=''.join(plotEleList2) ) )
...
def setworthhtml():
    ...
    for index, rows in csvdf1.iterrows():
        ...
        imgStr = '<h3 >2.{index} {name}策略: </h3> <img src="data:;base64,{imgStream}" ><br/>'.format(index=index+1, name=rows['策略名称'], imgStream=imgStream)
        plotEleList1.append(imgStr)
    ...
    for index, rows in csvdf2.iterrows():
        imgStream = getImgStream('%s/%s/%s.jpg' % (worthDir,'static', rows['策略名称']))
        imgStr = '<h3 >2.{index} {name}策略: </h3> <img src=src="data:;base64,{imgStream}" ><br/>'.format(name=rows['策略名称'], index=index+1, imgStream=imgStream)
        plotEleList2.append(imgStr)
