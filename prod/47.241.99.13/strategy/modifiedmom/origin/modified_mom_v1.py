...
def fun_polyfit(df_input,wid_,thd,if_print):
    ...
    df_s_upside = df_s.iloc[0:wid_,:]
    df_s_upside['abs_pct']=np.abs(df_s_upside['pct'])
    thd_pct = np.percentile(df_s_upside['abs_pct'],85)
    df_s_upside['pct']=np.where(df_s_upside['abs_pct']<thd_pct,df_s_upside['pct'],0)
    df_s_upside['pct'] = (1+df_s_upside['pct']).cumprod()

    mom = df_s_upside['pct'].iloc[-1]/df_s_upside['pct'].iloc[0] - 1
...
if __name__ == '__main__':
    ...
    wid_=np.arange(10*timespan) + 4*timespan
    thd = np.arange(1)*0.2+0
