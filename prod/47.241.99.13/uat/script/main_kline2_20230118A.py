...
class StrategyHandler(Handler):
    ...
    def handleTimer(self, data:TimeridType):
        ...
        logger.info(f"--{self.count=}--{hhmm=}--{diffTime=}--")

        if hhmm == 1840 and not self.flagDict['isOpen']:
            self.queryAccPx()
        ...
        elif hhmm == 1855 and not self.flagDict['isClose']:
            self.queryAccPx()
...
# nohup python3 -u main_kline8.py -n testkline8 -s 8889 -c 39095 -X BTCUSDT -p 3m -w 40 -d 3 -T 0.5 -t 5000  >> log_kline8.log 2>&1 &
