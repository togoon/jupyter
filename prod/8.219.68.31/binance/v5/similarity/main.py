...
pd.set_option('display.max_columns', 80)	# 显示列数
...
totalAdjRate = -1e-2  # -1e-3  -3e-3  -1e-2
...
print('\n--init--: ', f'{version=}, {gateway=}, {name=}, {symbol=}, {interval=}, {intervalCoef=}, {intervalSec=}, {tradeType=}, {histDays=},{limit=}', '\n')
...
class StrategyHandler(Handler):
    def __init__(self, name, symbol, total):
        ...
        self.flagDict = {'side':"", 'positionside':2, 'posBuy':0, 'posSell':0,'isNewDay':False, 'isOpenBuy':False, 'isOpenSell':False, 'isCloseBuy':False, 'isCloseSell':False, 'isOpen':False, 'isClose':False,'isTrig':False,'isCorr':False } 
    ...
    def handleKline(self, data:KlineType):
        ...
        print('--handleKline--: ', f"{version=}, {self.name=}, {self.symbol=}, {interval=}, {intervalSec=}, {wid=}, {histDays=}, {thd=}, {self.sign=}, {self.total=}, {self.flagDict['side']=}, {self.tradeCount=}, {self.count=}")
    ...
    def queryPositions(self):
        ...
        print((f"queryPositions_short: {self.symbol=}, {positions_short=}"))
    ...
    def savePickle(self, src_df, dest_df, flag):
        ...
        global name
        pklFile = f"{os.getcwd()}/pkl/{flag}_name.pkl"
    ...
    def handlebackTrade(self):
        ...
        print((f"handlebackTrade > queryPositions: {result=}")) 
        if result:
            for posi in result:
    ...
    def closeBuy(self):
        ...
        result = self.queryPositions()
        print((f"closeBuy > queryPositions: {result=}")) 
        if result:
            for posi in result:
    ...
    def closeSell(self):
        ...
        result = self.queryPositions()
        print((f"closeSell > queryPositions: {result=}"))       
        if result:
            for posi in result:
    ...
    def clearance(self):
        ...
        result = self.queryPositions()
        print((f"clearance > queryPositions: {result=}"))     
        if result:
            for posi in result:
...
def run(argv):
    ...
    input_pro = '-n <name> -s <serverPort> -c <clientPort> -X <symbol> -p <period> -w <wid> -d <day> -t <total> -r <adjRate>'
    try:
        # 优先级i>d>args 
        opts, args = getopt.getopt(argv, "n:s:c:X:p:w:d:t:r:",
                        ["name",'serverPort','clientPort',"symbol","period","wid","day","total","adjRate"])
        print(f'{opts=}')
    ...
    for opt, arg in opts:
        ...
        elif opt in ("-r", '--adjRate'):
            totalAdjRate = float(arg) * -1
...
# nohup python3 -u main.py -n similarity -s 8889 -c 39801 -X BTCUSDT -p 10m -w 42 -d 84 -t 15000  >> log.txt 2>&1  &
